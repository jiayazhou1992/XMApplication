...
public final class StringUtils {
	 ...
	 * @return
	 */
	public static SpannableStringBuilder getString(String str, @ColorRes int color, @DimenRes int size, int style) {
		if (StringUtils.isEmpty(str)) return new SpannableStringBuilder("");
		SpannableStringBuilder builder = new SpannableStringBuilder();
		builder.append(str);
		if (color != -1)
			builder.setSpan(new ForegroundColorSpan(ResourceUtils.getColor(color)), 0, str.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		if (style != -1)
			builder.setSpan(new StyleSpan(style), 0, str.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		if (size != -1)
			builder.setSpan(new AbsoluteSizeSpan(DevUtils.getContext().getResources().getDimensionPixelSize(size), false), 0, str.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		return builder;
	}
	 ...
	 * @return
	 */
	public static CharSequence toColorKey(String key, String content, @ColorRes int color){
		if (isEmpty(key) || isEmpty(content)) return content;
		...
		while (matcher.find()) {
			...
			SpannableStringBuilder newChar = getString(oldChar, color, -1, -1);
			int index = content.indexOf(oldChar, fromIndex);
			fromIndex = index + newChar.length();
			builder.delete(index, index + oldChar.length());
			builder.insert(index, newChar);
		}
		return builder;
	}

	public static CharSequence toColorKey2(CharSequence content, @ColorRes int color){
		if (content == null || content.length() == 0) return content;
		SpannableStringBuilder builder = new SpannableStringBuilder(content);

		String regex = "(?<=@tax-highlighted@).*?(?=@/tax-highlighted@)";
		Pattern pattern = Pattern.compile(regex);
		Matcher matcher = pattern.matcher(content);
		int fromIndex = 0;
		//Log.i(TAG,regex);
		while (matcher.find()) {
			String oldChar = matcher.group();
			SpannableStringBuilder newChar = getString(oldChar, color, -1, -1);
			//Log.i(TAG, " oldChar " + oldChar);
			String allOldChar = "@tax-highlighted@" + oldChar + "@/tax-highlighted@";
			int index = builder.toString().indexOf(allOldChar, fromIndex);
			fromIndex = index + newChar.length();
			builder.delete(index, index + allOldChar.length());
			builder.insert(index, newChar);
		}
		return builder;
	}

	public static CharSequence toColorKey3(CharSequence content, @ColorRes int color){
		if (content == null || content.length() == 0) return content;
		SpannableStringBuilder builder = new SpannableStringBuilder(content);
		...
		while (matcher.find()) {
			...
			SpannableStringBuilder newChar = getString(oldChar, color, -1, -1);
			//Log.i(TAG, " oldChar " + oldChar);
			String allOldChar = "<em>" + oldChar + "</em>";
			int index = builder.toString().indexOf(allOldChar, fromIndex);
			fromIndex = index + newChar.length();
			builder.delete(index, index + allOldChar.length());
			builder.insert(index, newChar);
	 ...
	 * @return
	 */
	public static String convertStreamToString(InputStream is, boolean closeInput) {
