...
//	System.out.println("时间差: " + timeDiff);
	 ...
	 * @param days
	 */
	public static String getAterTimeDays(String time, int days) {
		SimpleDateFormat simple = new SimpleDateFormat("yyyy-MM-dd");
		Date paseDate = null;
		try {
			paseDate = simple.parse(time);

			DevLogger.i("time 2:" + time);
			long newDate = paseDate.getTime() + days * 3600 * 24 * 1000L;
			String strDate = simple.format(new Date(newDate));
			DevLogger.i("time 2:" + strDate);
			return strDate;
		} catch (Exception e) {
			DevLogger.e("time 2:" + e.getMessage());
			return time;
		}
