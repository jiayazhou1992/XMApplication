...
public final class BitmapExtendUtils {
     ...
     * @param oldBitmap  要压缩的图片
     * @param maxSize 压缩后的大小，单位kb
     */
    public static Bitmap compress(Bitmap oldBitmap, double maxSize) {
        Bitmap newBitmap = scale(oldBitmap, 1);
        // 将bitmap放至数组中，意在获取bitmap的大小(与实际读取的原文件要大)
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        // 格式、质量、输出流
        oldBitmap.compress(Bitmap.CompressFormat.PNG, 70, baos);
        ...
        if (i > 1) {
            ...
            Bitmap bitmap = scale(newBitmap, newBitmap.getWidth() / Math.sqrt(i),newBitmap.getHeight() / Math.sqrt(i));
            newBitmap.recycle();
            newBitmap = bitmap;
        }
        return newBitmap;
