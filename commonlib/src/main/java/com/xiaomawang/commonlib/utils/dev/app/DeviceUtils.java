...
//    android.os.Build.VERSION.SDK_INT：系统的API级别 数字表示
    ...
    public static String getDeviceId(){
        String device_id = SharedUtils.getString(BaseConstans.device_id);
        if (StringUtils.isEmpty(device_id)){
            DevLogger.wTag("DEVICEID","there no device id");
            device_id = AppCommonUtils.getUUID();
            if (StringUtils.isEmpty(device_id)){
                DevLogger.wTag("DEVICEID","device id get fail");
                device_id = AppCommonUtils.getRandomUUID();
            }
            SharedUtils.put(BaseConstans.device_id,device_id);
        }
        return device_id;
    }
}
