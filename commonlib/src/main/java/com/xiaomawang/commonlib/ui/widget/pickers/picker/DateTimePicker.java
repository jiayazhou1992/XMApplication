...
import com.xiaomawang.commonlib.ui.widget.pickers.util.DateUtils;
...
public class DateTimePicker extends WheelPicker {
    ...
    protected View makeCenterView() {
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) && years.size() == 0) {
         //   LogUtils.verbose(this, "init1 years before make view");
            initYearData();
        }
        if (dateMode != NONE && months.size() == 0) {
            //     LogUtils.verbose(this, "init1 months before make view");
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) && days.size() == 0) {
            //    LogUtils.verbose(this, "init1 days before make view");
            int selectedYear;
            if (dateMode == YEAR_MONTH_DAY) {
        ...
        if (timeMode != NONE && hours.size() == 0) {
            //      LogUtils.verbose(this, "init1 hours before make view");
            initHourData();
        }
        if (timeMode != NONE && minutes.size() == 0) {
            //      LogUtils.verbose(this, "init1 minutes before make view");
